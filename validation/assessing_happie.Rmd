...
```{r}
happie_dat <- read.csv("combined_happie_coords", sep="\t", header = F, col.names = c("path", "program", "type", "sequence", "start", "stop"), stringsAsFactors = F)
...
new_dat <- new_dat %>% fill(host, ftp, assembly) 
new_dat$short_name <- gsub("\\(", "", gsub("\\)", "", gsub(" ", "_", new_dat$host )))
(new_dat$short_name %in% happie_dat$short_name)
