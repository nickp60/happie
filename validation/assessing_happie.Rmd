...
new_dat <- new_dat %>% group_by(short_name) %>%
  mutate(
    result = ifelse(
      (status %in% c("remnants", "partial", "intact", "intactish") & !id %in% c("na")) , 
      "True Positives",
      ifelse(
...
ggplot(new_dat, aes(x=status)) + geom_histogram(stat="count") + coord_flip()
ggplot(new_dat, aes(x=id)) + geom_histogram(stat="count") + coord_flip()
