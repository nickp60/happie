...
mdat <-left_join(x=new_dat, y=nested_happie, by=c("short_name", "sequence"))
#merged
...
for (i in 1:nrow(mdat)){
  dimob_vec <- as.logical(d[i, "unfiltered_dimob_hit"][[1]][[1]])
  prophet_vec <- as.logical(d[i, "unfiltered_prophet_hit"][[1]][[1]])
...
```{r}
keepers <- c("short_name","sequence","assessment","mynotes","status", "valid_locus_start",   "valid_locus_end" ,"prophet_hit", "dimob_hit")
mdat <- mdat%>% select(keepers) %>%  View()
```
...
ggplot(mdat, aes(x=id, fill=id)) + geom_histogram(stat="count") + coord_flip() + scale_fill_manual(values = c("grey80", "grey60", "grey40", "grey20"), guide=F)
